...
<div class="max-w-5xl mx-auto w-full">
    ...
    <div class="card-tw shadow-sm">
        <div class="card-header-tw flex items-center justify-between">
            ...
            <span class="badge-tw badge-info-tw hidden sm:inline-flex">Step 7 of 7</span>
        </div>
        ...
        <form action="{{ route('admin.manage-property.update-property-documents', ['id' => $property_id]) }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                @for ($i = 1; $i <= 5; $i++)
                    @php $docKey = 'document_' . $i; @endphp
                    <div class="p-4 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50 space-y-2">
                        <label class="block text-xs font-semibold uppercase tracking-wider text-slate-600 dark:text-slate-300" for="{{ $docKey }}">
                            Legal Document Tier {{ $i }} (PDF)
                        </label>
                        <input type="file" class="input-tw file:mr-3 file:py-1 file:px-2.5 file:rounded-md file:border-0 file:text-xs file:font-semibold file:bg-indigo-50 file:text-indigo-700 dark:file:bg-indigo-950 dark:file:text-indigo-300" accept="application/pdf" name="{{ $docKey }}" id="{{ $docKey }}">
                        <p class="text-[11px] text-slate-400">LLC Operating Agreement or Property Deed.</p>
                    </div>
                @endfor
                ...
                <div class="p-4 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50 space-y-2">
                    ...
                    <p class="text-[11px] text-slate-400">Detailed line-item OPEX forecast.</p>
                </div>
                ...
                <div class="p-4 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50 space-y-2">
                    ...
                    <p class="text-[11px] text-slate-400">Historical ADR and occupancy pro-forma.</p>
                </div>
            ...
            <div class="border-t border-slate-200 dark:border-slate-800 pt-5 flex flex-col sm:flex-row items-center justify-between gap-3">
                ...
                <div class="flex items-center gap-2.5 w-full sm:w-auto order-1 sm:order-2">
                    <a href="{{ route('manage-property.index') }}" class="btn-secondary-tw w-full sm:w-auto text-center">
                        <i class="bi bi-check2-all"></i> Finish & Return to List
                    </a>
                    <button type="submit" class="btn-primary-tw w-full sm:w-auto flex items-center justify-center gap-2">
                        <i class="bi bi-cloud-arrow-up text-base"></i> Upload & Save Documents
                    </button>
                </div>
            </div>
        </form>
