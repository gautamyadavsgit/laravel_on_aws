    ...
    <div class="max-w-md w-full mx-auto px-4 sm:px-6">
        <div class="card-tw p-8 sm:p-10 space-y-6 shadow-xl relative overflow-hidden">
            ...
            <div class="text-center space-y-2">
                ...
                <h1 class="text-2xl font-bold tracking-tight text-slate-900 dark:text-white">Investor Login</h1>
                <p class="text-sm text-slate-500 dark:text-slate-400">Access your fractional deed portfolio and dividend reports.</p>
            </div>
            ...
            <div class="text-center pt-3 border-t border-slate-100 dark:border-slate-800 text-xs text-slate-600 dark:text-slate-400">
                ...
                <a href="{{ route('register') }}" class="font-bold text-indigo-600 hover:text-indigo-500 dark:text-indigo-400 dark:hover:text-indigo-300 hover:underline ml-1">
                    Create an investor account &rarr;
                </a>
            </div>

            <!-- Admin Login Link -->
            <div class="text-center text-[11px] text-slate-400 dark:text-slate-500">
                Are you an administrator? <a href="{{ route('admin.index') }}" class="hover:underline text-slate-500 dark:text-slate-400">Admin Portal</a>
            </div>
        </div>
    </div>
</div>
...
document.getElementById('loginForm')?.addEventListener('submit', function() {
    ...
    if (btn) {
        btn.disabled = true;
        btn.innerHTML = '<span class="inline-block animate-spin mr-2"><i class="bi bi-arrow-repeat"></i></span> Signing In...';
    }
});
</script>
